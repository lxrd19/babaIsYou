...
void ManageData::findVerticalRules(const std::vector<std::vector<Box>>& board,
    ...
    for(const auto& pos : isPosition){
        ...
        if(isInside(board, up) && isInside(board, down) && box1.isOccupied() && box2.isOccupied()){
            ...
            if(!element.empty()){
                if(effect == "you"){
                    rules.emplace_back(std::make_unique<IsYou>(element/*, positions*/, m_game));
                }else if (effect == "push"){
                    rules.emplace_back(std::make_unique<IsPush>(element/*, positions*/, m_game));
                }else if (effect == "kill"){
                    rules.emplace_back(std::make_unique<IsKill>(element/*, positions*/, m_game));
                }else if (effect == "sink"){
                    rules.emplace_back(std::make_unique<IsSink>(element/*, positions*/, m_game));
                }else if (effect == "win"){
                    rules.emplace_back(std::make_unique<IsWin>(element/*, positions*/, m_game));
                }else if (effect == "stop"){
                    rules.emplace_back(std::make_unique<IsStop>(element/*, positions*/, m_game));
                }
            }
